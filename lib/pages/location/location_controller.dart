...
//       final size = Size(180, 250); // 180头像 + 顶部50标签 + 底部余量
      
//       // 先绘制底座（在头像背后）
//       final pedestal = await _loadImageFromAsset(baseAsset);
//       if (pedestal != null) {
//         final imageSize = Size(pedestal.width.toDouble(), pedestal.height.toDouble());
//         final scaleX = size.width / imageSize.width; // 以宽度等比缩放
//         final scaledWidth = imageSize.width * scaleX;
//         final scaledHeight = imageSize.height * scaleX;
//         final offsetX = (size.width - scaledWidth) / 2;
//         final offsetY = 50 + ((size.height - 50) - scaledHeight); // 底对齐，排除顶部标签区
...
//       // 顶部预留50用于标签，底部再预留一点空间用于底座遮挡
//       final avatarCenter = Offset(90, 140); // 50 + 半径90 = 140，略微上移避免被底座遮挡过多
...
//       final recorder = ui.PictureRecorder();
//       final canvas = Canvas(recorder);
//       final size = Size(180, 200); // 180头像 + 底部留白用于底座
...
//         final scaleX = size.width / imageSize.width; // 以宽度等比缩放
//         final scaledWidth = imageSize.width * scaleX;
//         final scaledHeight = imageSize.height * scaleX; // 以宽度缩放
//         // 我的标记：底座右边缘中心点与坐标点重合
//         final offsetX = size.width - scaledWidth; // 右对齐，让底座右边缘中心点与坐标点重合
//         final offsetY = size.height - scaledHeight; // 底对齐
//         final srcRect = Rect.fromLTWH(0, 0, imageSize.width, imageSize.height);
//         final dstRect = Rect.fromLTWH(offsetX, offsetY, scaledWidth, scaledHeight);
//         canvas.drawImageRect(pedestal, srcRect, dstRect, Paint());
//       }
...
//       // 绘制圆形头像 - 180x180
//       final avatarSize = 180.0;
//       final avatarCenter = Offset(90, 90); // 头像底部与坐标点重合（90 + 90 = 180，正好是头像底部）
...
//               baseAsset: 'assets/3.0/kissu3_location_mine.webp',
//             );
//           } else {
//             // 未绑定状态：判断当前位置是否代表另一半
...
//             icon: myIcon,
//             anchor: const Offset(1.0, 1.0), // 我的：底座右边缘中心点与坐标重合，头像底部与坐标重合
...
//             icon: partnerIcon,
//             anchor: const Offset(0.5, 1.0), // 另一半：底座中心点与坐标重合，头像底部与坐标重合
//             onTap: (String markerId) {
